...
public class SearchGridAdapter extends BaseAdapter {
    ...
    private ArrayList<SearchDatum> productBySubCatItemList;
    private Activity activity;
    private OnViewClick viewClick;
    private String loginValue = "";
    private SharedPreferences pref;

    public SearchGridAdapter(Activity activity, ArrayList<SearchDatum> productBySubCatItemList) {
    ...
    public View getView(int position, @Nullable View convertView, @NonNull ViewGroup parent) {
        ...
        Glide.with(activity).load(productBySubCatItemList.get(position).getProductFiles().get(0).getUrl()).placeholder(R.drawable.logo).into(img_item);
        ...
        linear_item.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                String data = new Gson().toJson(productBySubCatItemList.get(position));
